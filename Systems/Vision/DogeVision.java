...
public class DogeVision extends DogeCVDetector {
  ...
  private Mat maskYellow = new Mat(); // DogeVision Mask returned by color filter
  private Mat hierarchy = new Mat(); // hierarchy used by coutnours

  // Results of the detector
  public boolean found = false; // Is the gold mineral found
  public boolean aligned = false; // Is the gold mineral aligned
  public double xPos = 0, yPos = 0, objectWidth = 0, objectHeight = 0;
  ...
  public void initDetector(HardwareMap hardwareMap) {
    ...
    ratioScorer.weight = 5; // Weight Ratio
  ...
  public Mat process(Mat input) {
    ...
    if (debugAlignment) {
      ...
      if (isFound()) {
        Imgproc.line(displayMat, new Point(xPos, getAdjustedSize().height), new Point(xPos, getAdjustedSize().height - 30), new Scalar(255, 255, 0), 2);
      }
